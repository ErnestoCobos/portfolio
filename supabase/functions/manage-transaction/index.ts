...
Deno.serve(async req => {
  ...
  try {
    ...
    if (action === 'delete' && transaction_id) {
      // Get the transaction before deleting to reverse its balance effect
      const { data: oldTransaction } = await supabaseClient
        .from('transactions')
    ...
    if (action === 'update' && transaction_id) {
      ...
      // Adjust balance: reverse old transaction effect, then apply new transaction
      if (oldTransaction) {
        // Reverse old transaction's effect on its original account
        ...
        // Apply new transaction's effect on the (potentially different) account
